...
class MuseEngine extends Engine {
   ...
   */
  constructor({ address, controlEngine, incDecAmount = INC_DEC_AMOUNT, smaPeriod = SMA_PERIOD }) {
    super();
  ...
  sendFirstMessage(normalized) {
    this.controlEngine.update(normalized);
  ...
  send(value) {
    ...
    if (this.firstMessageState.isWaiting) {
      ...
      this.controlEngine.rampTo(this.latestNormalized);
      ...
      this.controlEngine.update(this.latestNormalized);
    }
  }
