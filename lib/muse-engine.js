...
class MuseEngine extends Engine {
  ...
  sendFirstMessage(normalized) {
    this.controllerEngine.update(normalized);
  ...
  send(value) {
    ...
    if (this.firstMessageState.isWaiting) {
      ...
      this.controllerEngine.rampTo(this.latestNormalized);
      ...
      this.controllerEngine.update(this.latestNormalized);
    }
  }
  ...
  reset(channelMap) {
    ...
    this.controllerEngine.reset(channel);
  }
}
