...
const buildEngineMap = () => {
  ...
  const alphaEngines = [1, 2, 3, 4].map((i) => new ControllerEngine({
    channel: getChannelForAddress(channelMap, ADDRESS_ALPHA),
    controllerNumber: i,
  }));

  const betaEngines = [1, 2, 3, 4].map((i) => new ControllerEngine({
    channel: getChannelForAddress(channelMap, ADDRESS_BETA),
    controllerNumber: i,
  }));

  const thetaEngines = [1, 2, 3, 4].map((i) => new ControllerEngine({
    channel: getChannelForAddress(channelMap, ADDRESS_THETA),
    controllerNumber: i,
  }));

  const gammaEngines = [1, 2, 3, 4].map((i) => new ControllerEngine({
    channel: getChannelForAddress(channelMap, ADDRESS_GAMMA),
    controllerNumber: i,
    isActive: false,
  }));
  ...
  engineMap.set(
    ADDRESS_ALPHA,
    new MuseEngine({ address: ADDRESS_ALPHA, controllerEngines: alphaEngines, channelMap })
  );
  engineMap.set(
    ADDRESS_BETA,
    new MuseEngine({ address: ADDRESS_BETA, controllerEngines: betaEngines, channelMap })
  );
  engineMap.set(
    ADDRESS_THETA,
    new MuseEngine({ address: ADDRESS_THETA, controllerEngines: thetaEngines, channelMap })
  );
  engineMap.set(
    ADDRESS_GAMMA,
    new MuseEngine({ address: ADDRESS_GAMMA, controllerEngines: gammaEngines, channelMap })
  );
